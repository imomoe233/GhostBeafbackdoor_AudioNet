...
class ePGD(FGSM):
    ...
    def __init__(self, model, task='CSI', epsilon=0.002, max_iter=10, num_random_init=0,
        ...
        if self.task in ['SV', 'OSI']:
            ...
            print('Running white box attack for {} task, directly using the true threshold {}'.format(self.task,
                                                                                                      self.threshold))
    ...
    def attack_batch(self, x_batch, y_batch, lower, upper, batch_id):
        ...
        for ti in Ti:
            for t_cur in range(ti):
                alpha_list.append(alpha_max * (gamma ** t_cur))
        alpha_list1 = alpha_list[7:17]
        alpha_list2 = alpha_list[9:29]
        alpha_list3 = alpha_list[10:40]
        alpha_list4 = alpha_list[1:11] #hard csi osi
        alpha_list5 = alpha_list[2:22]
        alpha_list6 = alpha_list[3:33]

        alpha_list7 = alpha_list[2:12] #imposter10
        alpha_list8 = alpha_list[3:23] #imposter20
        alpha_list9 = alpha_list[6:36]
        for iter in range(self.max_iter + 1):
            ...
            if self.verbose:
                print("batch:{} iter:{} loss: {} predict: {}, target: {}".format(batch_id, iter,
            ...
            if iter < self.max_iter:
                ...
                x_batch.data += alpha_list3[iter-1] * torch.sign(x_batch.grad) * self.grad_sign
                x_batch.grad.zero_()

                Upper = torch.clamp(x_batch + self.epsilon, max=upper)
                Lower = torch.clamp(x_batch - self.epsilon, min=lower)
                x_batch.data = torch.min(torch.max(x_batch.data, Lower), Upper)  # 对抗音频限制 包括你算法中的两个公式
        ...
        if flags:
            x_batch = adv_recoder
        att_time = 0
        return x_batch, success, att_time
    ...
    def attack(self, x, y):
        ...
        for init in range(max(1, self.num_random_init)):
            ...
            for batch_id in range(n_batches):
                ...
                adver_x_batch, success_batch, att_time = self.attack_batch(x_batch, y_batch, lower_batch, upper_batch,
