...
class CW2(FGSM):
    ...
    def __init__(self, model, task='CSI',
        ...
        self.loss = SEC4SR_MarginLoss(targeted=self.targeted, confidence=self.confidence, task=self.task,
                                      threshold=self.threshold, clip_max=True)
    ...
    def attack_batch(self, x_batch, y_batch, lower, upper, batch_id):
        ...
        global_best_score = [
                                -2] * n_audios  # do not use [-1] * n_audios since -1 is within the decision space of SV and OSI tasks
        ...
        for _ in range(self.binary_search_steps):
            ...
            for n_iter in range(self.max_iter + 1):
                ...
                for ii, (l2, y_pred, adver_x, l1) in enumerate(zip(loss2, predict, input_x, loss1)):
                    # IF-BRANCH-1
                    if l1 <= 0 and l2 < best_l2[
                        ii]:  # l1 <= 0 indicates the attack succeed with at least kappa confidence
                        best_l2[ii] = l2
                        best_score[ii] = y_pred
                    # IF-BRANCH-2
                    if l1 <= 0 and l2 < global_best_l2[
                        ii]:  # l1 <= 0 indicates the attack succeed with at least kappa confidence
